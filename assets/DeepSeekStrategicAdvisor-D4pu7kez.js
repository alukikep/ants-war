var k=Object.defineProperty;var w=(s,e,a)=>e in s?k(s,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):s[e]=a;var m=(s,e,a)=>w(s,typeof e!="symbol"?e+"":e,a);import{H as O,T as D,A as $,P as H}from"./index-DqzkL17T.js";const C=["upgrade_focus","build_focus","iterate"];function S(s,e){const a=e.stats,i=e.type,n=i==="head"?"H":i==="thorax"?"T":"A",l=[];a.damage&&l.push(`攻${_(a.damage)}`),a.hp&&l.push(`血${_(a.hp)}`),a.speed&&l.push(`速${_(a.speed)}`),a.attackSpeed&&l.push(`攻速${_(a.attackSpeed)}%`);const t=[];i==="head"&&(s==="leafcutter"&&t.push("crit1:5/2:10/3:15"),s==="fire"&&t.push("双生:同格+1只"),s==="odontomachus"&&t.push("逃:回血50% <40%血 10s cd"),s==="termiteSoldier"&&t.push("光环:加攻速 至300% 衰减20%/s"),s==="bigHead"&&t.push("秒:3/5/8"),s==="soldier"&&t.push("重装:高攻慢速")),i==="thorax"&&(s==="carpenter"&&a.flatArmor&&t.push(`甲+${a.flatArmor}`),s==="leafcutter"&&t.push("嘲:<20%血 +30%hp +80%甲5s 15s cd"),s==="bullet"&&t.push("肾:首次受敌 +攻/甲 5/8/12s 60s cd")),i==="abdomen"&&(s==="spitter"&&(t.push("远攻+25"),t.push("血-30%"),t.push("慢2:20/3:40")),s==="honeypot"&&t.push("死爆:+50hp回血 半径80"),s==="matabele"&&t.push("毒针5s cd:减攻速50%+毒40/60/100"),s==="weaver"&&t.push("均衡:血/速/攻速"),s==="trap"&&t.push("爆发:攻+攻速"));const c=l.length?l.join(" "):"无加成",b=t.length?" "+t.join(" "):"";return`${n}:${s}(${e.nameCN}) 价${e.cost} ${c}${b}`}function _(s){return`${s>0?"+":""}${s}`}function N(s,e,a){const i=s.map(t=>S(t,O[t])).join(`
`),n=e.map(t=>S(t,D[t])).join(`
`),l=a.map(t=>S(t,$[t])).join(`
